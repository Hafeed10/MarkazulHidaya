      ...
      <div className="flex items-center justify-between   w-full h-full">
        ...
        <ul className="xl:flex hidden space-x-7">
          {navigations.map((navigation, index) => (
            <li
              key={index}
              className={`font-medium  text-sm xl:text-base hover:text-[#CCA65D] text-${fix ? '[#555555]' : 'white'} font-sans tracking-wider uppercase transition-all duration-200 ease-in cursor-pointer`}
            >
              <Link
                activeClass="active"
                to={navigation.path}
